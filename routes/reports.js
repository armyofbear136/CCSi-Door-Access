...
reportsRouter.post('/', async function (req, res, next) {
  ...
  if (doorsReport && req.body.logtype == "event"){
  ...
  //generate csv for door access report 

  else if (doorsReport && req.body.logtype == 'access'){
  ...
  // console.log(parsedAllEvents)
    ...
    for (var e = 1; e < parsedAllEvents.card_events.length; e++)
    {
      var doorMatch = false;
      let cardEvent = parsedAllEvents.card_events[e];
      console.log(cardEvent);
      if ((new Date(cardEvent[2]) > (startDT)) && (new Date(cardEvent[2]) < (endDT)))
      {
        for (d in doorObjects){
          if (doorObjects[d].reader == cardEvent[3]){
            if (req.body.dooronlygroups.includes(d)){doorMatch = true;}
        ...
        if (doorMatch){
          selectedDoorEvents.push(cardEvent);
        }
      }
    }
  // console.log(selectedUserEvents);
  console.log('generating door access report');
  await CSVfun.generate((reportTitle + '/' + req.body.name + '_' + 'DoorAccess_' + Date.now() + '.csv'), selectedDoorEvents);
}
...
/* GET report page. */
reportsRouter.get('/view/:reportID', async function (req, res, next) {
